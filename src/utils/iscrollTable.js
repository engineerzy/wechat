...
export function createIScroller(selector) {
  ...
  function updatePosition() {
    ...
    for (let i = 0; i < frozenCols.length; i++) {
      frozenCols[i].style.transform = 'translate(' + -1 * this.x/100 + 'rem, 0px) translateZ(0px)';
    }
    for (let i = 0; i < frozenRows.length; i++) {
      frozenRows[i].style.transform = 'translate(0px, ' + -1 * this.y/100 + 'rem) translateZ(0px)';
    }
    for (let i = 0; i < frozenCrosses.length; i++) {
      frozenCrosses[i].style.transform = 'translate(' + -1 * this.x/100 + 'rem,' + -1 * this.y/100 + 'rem) translateZ(0px)';
    }
  }
