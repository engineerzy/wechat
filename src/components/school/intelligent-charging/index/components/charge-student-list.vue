<template>
  <div class="charge-student-list">
    <more-tabs class="class-tabs" v-model="currentClassIndex" :list="classNameList" size="small" />

    <div class="table-head">
      <van-row>
        <van-col @click.native="sort('index')" :class="{ currentSortColumn: sortColName === 'index' }" span="4">序号</van-col>
        <van-col @click.native="sort('name')" :class="{ currentSortColumn: sortColName === 'name' }" span="4">姓名</van-col>
        <van-col @click.native="sort('bindTime')" :class="{ currentSortColumn: sortColName === 'bindTime' }" :span="timeColSpan">绑定时间</van-col>
        <van-col @click.native="sort('unbindTime')" :class="{ currentSortColumn: sortColName === 'unbindTime' }" :span="timeColSpan">删除时间</van-col>
        <van-col @click.native="sort('amount')" :class="{ currentSortColumn: sortColName === 'amount' }" v-if="!isMinimumAmount" span="4">费用</van-col>
      </van-row>
    </div>
    <van-list class="table-body" :finished="studentList.length > 10" finished-text="没有更多了">
      <div class="table-content">
        <van-row v-for="(student, index) in studentList" :key="index">
          <van-col span="4">{{ student.index }}</van-col>
          <van-col :class="{ red: student.status !== 1 }" span="4">{{ student.name }}</van-col>
          <van-col :span="timeColSpan">{{ student.bindTimeText }}</van-col>
          <van-col :span="timeColSpan">{{ student.unbindTimeText }}</van-col>
          <van-col v-if="!isMinimumAmount" span="4">{{ student.amount }}元</van-col>
        </van-row>
      </div>
    </van-list>
  </div>
